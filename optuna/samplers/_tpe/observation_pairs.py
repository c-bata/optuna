...
def _get_observation_pairs_univariate_single_objective(
    study: Study,
    param_name: str,
...
def _get_observation_pairs_univariate_multi_objective(
    study: Study,
    param_name: str,
    ...
    for trial in study.get_trials(deepcopy=False, states=(TrialState.COMPLETE,)):
        if trial.values is None:
            continue
        scores.append((-float("inf"), [sign * v for sign, v in zip(signs, trial.values)]))
...
def _get_observation_pairs_univariate_single_objective_constraints(
    study: Study,
    param_name: str,
    constant_liar: bool = False,  # TODO(hvy): Remove default value and fix unit tests.
...
def _get_observation_pairs_univariate_multi_objective_constraints(
        study: Study,
        param_name: str,
        constant_liar: bool = False,  # TODO(hvy): Remove default value and fix unit tests.
        constraints_enabled: bool = False,
