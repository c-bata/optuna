...
class CmaEsSampler(BaseSampler):
    ...
    def sample_relative(
        ...
        if optimizer is None:
            n_restarts = 0
            optimizer = self._init_optimizer(transform)
        ...
        if optimizer.dim != transform.bounds.shape[0]:
            _logger.info(
        ...
        if len(solution_trials) >= optimizer.population_size:
            ...
            for t in solution_trials[: optimizer.population_size]:
                ...
                x = transform.transform(t.params)
            ...
            if self._restart_strategy == "ipop" and optimizer.should_stop():
                ...
                optimizer = self._init_optimizer(
                    transform, population_size=popsize, randomize_start_point=True
                )
        ...
        external_values = transform.untransform(params)
    ...
    def _init_optimizer(
        ...
        lower_bounds = transform.bounds[:, 0]
        upper_bounds = transform.bounds[:, 1]

        if randomize_start_point:
            mean = np.empty(shape=len(lower_bounds), dtype=float)
            for i, (lb, ub) in enumerate(zip(lower_bounds, upper_bounds)):
                mean[i] = self._cma_rng.uniform(lb, ub)
        elif self._x0 is None:
            mean = lower_bounds + (upper_bounds - lower_bounds) / 2
        else:
            # `self._x0` is external representations.
            mean = transform.transform(self._x0)

        if self._sigma0 is None:
            sigma0 = np.min((upper_bounds - lower_bounds) / 6)
        else:
            sigma0 = self._sigma0
        sigma0 = max(sigma0, _EPS)  # Avoid ZeroDivisionError in cmaes

        # UniformDistribution and LogUniformDistribution do not accept the value
        # which equals to the upper bound.
        bounds = copy.deepcopy(transform.bounds)
        bounds[:, 1] -= _EPS
        n_dimension = bounds.shape[0]
